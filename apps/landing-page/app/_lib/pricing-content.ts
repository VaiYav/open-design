...
const EN: PricingContent = {
  labels: {
    ...
    creditBenefit: '{amount} model credits / mo · text, image & video',
    creditBonus: 'Limited +{pct}% bonus',
    multimodalBenefit: 'Open Design Cloud image & video generation',
    multimodalTitle: 'One credit balance for text, image, and video',
    multimodalDescription: 'Use Open Design Cloud models without configuring provider API keys. See an estimate before generation; successful generations are charged by actual usage.',
...
const ZH_CN: PricingContent = {
  labels: {
    ...
    standardModels: '标准模型',
    recommended: '推荐',
    creditBenefit: '每月 {amount} 模型额度 · 文本、图片与视频通用',
    creditBonus: '限时加赠 {pct}%',
    multimodalBenefit: 'Open Design 云端图片与视频生成',
    multimodalTitle: '一份模型额度，覆盖文本、图片与视频',
    multimodalDescription: '无需分别配置供应商 API Key。生成前展示预估费用，成功后按实际用量扣除。',
    textGeneration: '文本生成',
...
const ZH_TW: PricingContent = {
  labels: {
    ...
    standardModels: '標準模型',
    recommended: '推薦',
    creditBenefit: '每月 {amount} 模型額度 · 文字、圖片與影片通用',
    creditBonus: '限時加贈 {pct}%',
    multimodalBenefit: 'Open Design 雲端圖片與影片生成',
    multimodalTitle: '一份模型額度，涵蓋文字、圖片與影片',
    multimodalDescription: '無需分別配置供應商 API Key。生成前顯示預估費用，成功後依實際用量扣除。',
    textGeneration: '文字生成',
...
const ES: PricingContent = {
  labels: {
    ...
    creditBenefit: '{amount} en créditos / mes · texto, imagen y vídeo',
    creditBonus: '+{pct}% extra (limitado)',
    multimodalBenefit: 'Generación de imágenes y vídeo en Open Design Cloud',
    multimodalTitle: 'Un saldo para texto, imagen y vídeo',
    multimodalDescription: 'Usa modelos de Open Design Cloud sin configurar claves API de proveedores. Consulta una estimación antes de generar; solo se cobra el uso real de las generaciones completadas.',
...
const PT_BR: PricingContent = {
  labels: {
    ...
    recommended: 'Recomendado',
    creditBenefit: '{amount} em créditos / mês · texto, imagem e vídeo',
    creditBonus: '+{pct}% bônus (limitado)',
    multimodalBenefit: 'Geração de imagem e vídeo no Open Design Cloud',
    multimodalTitle: 'Um saldo para texto, imagem e vídeo',
    multimodalDescription: 'Use modelos do Open Design Cloud sem configurar chaves de API de provedores. Veja uma estimativa antes de gerar; gerações concluídas são cobradas pelo uso real.',
...
const RU: PricingContent = {
  labels: {
    ...
    creditBenefit: '{amount} кредитов / мес. · текст, изображения и видео',
    creditBonus: '+{pct}% бонус (ограничено)',
    multimodalBenefit: 'Генерация изображений и видео в Open Design Cloud',
    multimodalTitle: 'Единый баланс для текста, изображений и видео',
    multimodalDescription: 'Используйте модели Open Design Cloud без настройки API-ключей провайдеров. До генерации показывается оценка, а успешные генерации оплачиваются по фактическому использованию.',
...
const FR: PricingContent = {
  labels: {
    ...
    creditBenefit: '{amount} de crédits / mois · texte, image et vidéo',
    creditBonus: '+{pct}% bonus (limité)',
    multimodalBenefit: 'Génération d’images et de vidéos dans Open Design Cloud',
    multimodalTitle: 'Un seul solde pour le texte, l’image et la vidéo',
    multimodalDescription: 'Utilisez les modèles Open Design Cloud sans configurer de clés API fournisseur. Une estimation s’affiche avant la génération ; les générations réussies sont facturées selon l’usage réel.',
...
const KO: PricingContent = {
  labels: {
    ...
    recommended: '추천',
    creditBenefit: '매월 {amount} 모델 크레딧 · 텍스트, 이미지, 동영상 공용',
    creditBonus: '한정 {pct}% 추가 증정',
    multimodalBenefit: 'Open Design Cloud 이미지 및 동영상 생성',
    multimodalTitle: '하나의 크레딧으로 텍스트, 이미지, 동영상 생성',
    multimodalDescription: '공급자 API 키를 별도로 설정하지 않고 Open Design Cloud 모델을 사용하세요. 생성 전 예상 비용을 확인하고, 성공한 생성은 실제 사용량만큼 차감됩니다.',
...
const DE: PricingContent = {
  labels: {
    ...
    creditBenefit: '{amount} Credits / Monat · Text, Bild und Video',
    creditBonus: '+{pct}% Bonus (befristet)',
    multimodalBenefit: 'Bild- und Videogenerierung in Open Design Cloud',
    multimodalTitle: 'Ein Guthaben für Text, Bild und Video',
    multimodalDescription: 'Nutze Open Design Cloud-Modelle ohne separate Anbieter-API-Schlüssel. Vor der Generierung erscheint eine Schätzung; erfolgreiche Generierungen werden nach tatsächlicher Nutzung abgerechnet.',
...
const JA: PricingContent = {
  labels: {
    ...
    recommended: 'おすすめ',
    creditBenefit: '毎月 {amount} のモデルクレジット · テキスト・画像・動画共通',
    creditBonus: '期間限定 {pct}% 増量',
    multimodalBenefit: 'Open Design Cloud の画像・動画生成',
    multimodalTitle: '1つのクレジットでテキスト・画像・動画を生成',
    multimodalDescription: 'プロバイダーの API キーを個別に設定せず、Open Design Cloud のモデルを利用できます。生成前に見積もりを表示し、成功した生成は実際の使用量に応じて課金されます。',
